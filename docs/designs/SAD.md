...
`bamboo_spec_generator`의 핵심 아키텍처는 입력 어댑터, 검증/모델 변환, 스크립트 자산 렌더링, Bamboo Specs 생성, 산출물 기록, 빌드 메타데이터 저장, 운영 백엔드/API/조회 UI로 분리된다. 장기적으로는 JSON과 DB가 동일 내부 모델로 수렴하고, 생성 파이프라인은 입력 소스와 무관하게 공통 생성 로직을 유지해야 한다.

## 현재 구현 기준

- 현재 실제 구현된 입력 어댑터는 JSON loader 하나다.
- 현재 실제 구현된 생성 파이프라인은 `parser -> validator -> script asset/render -> generator -> writer` 흐름이다.
- DB 입력 어댑터, 메타데이터 저장 서비스, 버전 저장소, 실행 이력 저장소는 아직 구현되지 않았다.
- Django 운영 백엔드, Ninja API, 조회용 프론트엔드는 아직 구현되지 않았다.
...
### 5. Bamboo Specs 생성 계층

- 공통 워크플로우를 기준으로 Java Specs를 생성한다.
- 저장소 연결, 트리거, requirement, 스크립트 inline body 또는 보조 자산 출력을 담당한다.
...
2. Django 백엔드가 PostgreSQL에서 프로젝트/플랜/버전/실행 이력 조회
