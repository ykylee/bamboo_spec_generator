...
`bamboo_spec_generator`의 핵심 아키텍처는 입력 어댑터, 검증/모델 변환, 스크립트 자산 렌더링, Bamboo Specs 생성, 산출물 기록, 빌드 메타데이터 저장, 운영 백엔드/API/조회 UI로 분리된다. 장기적으로는 JSON과 DB가 동일 내부 모델로 수렴하고, 생성 파이프라인은 입력 소스와 무관하게 공통 생성 로직을 유지해야 하며, 시스템 범위는 빌드 관리에서 배포/릴리스 관리까지 확장되어야 한다.
