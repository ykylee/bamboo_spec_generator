...
연도별로 관리되는 하나 이상의 JSON 빌드 정의를 읽어, 여러 빌드에 대응하는 Bamboo Specs Java 코드를 하나의 Specs 저장소 안에 생성하는 애플리케이션을 구축한다.
...
시스템은 여러 JSON 파일을 입력으로 받아야 하며, 타겟 빌드는 연도별로 관리될 수 있어야 한다. 각 빌드 정의를 분석해 하나의 Bamboo Specs 저장소 안에서 여러 플랜을 함께 관리할 수 있는 Bamboo Specs Java 코드를 생성해야 한다. 또한 Atlassian에서 제공하는 Bamboo Specs Base 코드를 저장소에서 관리하고 생성 로직에 재사용할 수 있어야 한다.
