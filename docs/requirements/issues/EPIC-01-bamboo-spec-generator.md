...
- stage/job/task 모듈화와 CI 도구별 task 분리는 신규 요구사항으로 추가되었다.
...
연도별로 관리되는 하나 이상의 JSON 빌드 정의를 읽어, 여러 빌드에 대응하는 Bamboo Specs Java 코드를 하나의 Specs 저장소 안에 생성하고, Bitbucket 저장소 연결 및 브랜치 트리거 구성까지 함께 지원하는 애플리케이션을 구축한다. 또한 장기적으로는 build workflow를 `stage -> job -> task` 모듈 조합으로 분리하고, CI 도구별 task 확장을 수용할 수 있는 생성기 구조를 갖춰야 한다.
...
- Jenkins까지 고려한 task 모델 분리 수준을 어디까지 EPIC-01 범위로 볼지 결정 필요
